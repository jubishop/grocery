...
const asymmetricVariantGuards = [
  ...
  { pattern: /\bnatural\b/, context: /\bpeanut\s+butter\b/ },
  ...
  { pattern: /\bcreamy\b/, context: /^(?!.*\b(?:cheese sauce|macaroni)\b).*\bsauce\b/ },
  ...
  { pattern: /\bvariety\s+pack\b/, context: /\b(?:beverage|can|drink|energy|soda)\b/ },
  { pattern: /\bwhite\s+grape\b/, context: /\b(?:juice|drink|beverage)\b/ },
...
export function packagedProductVariantsCompatible(leftValue, rightValue) {
  ...
  const conflicts = [
    ...
    stateClaimConflict(
      ...
      /^(?!.*\b(?:breaded|cooked|gravy|marinade|nugget|prepared|sauce|seasoning|soup)\b).*\b(?:beef|bison|chicken|duck|lamb|pork|steak|turkey|veal)\b/,
    ),
